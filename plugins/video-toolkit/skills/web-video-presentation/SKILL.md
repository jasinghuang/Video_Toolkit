...
CSS 设计系统 + theme.json + design.md），12 个语义 class 作为章节与主题之间
的不变桥梁。配色、字型由主题承载，动效气质由章节 agent 自由发挥。
...
2. 章节代码物理分离（每章一个文件夹 / 自己的 CSS 前缀），不会互相
   破坏
3. 12 MUST class 和 CSS 变量兜底视觉统一（颜色 / 字体 / stat-value
   / card / divider 等组件由主题控制），气质不会跑偏
...
| 9 | 整片同一主题 | 章节间不翻表面色；**颜色走 CSS 变量、组件走 MUST class**，动画 / 字号 / 间距章节自由 |
