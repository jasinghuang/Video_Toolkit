...
description: 把文章或口播稿做成点击驱动的 16:9 网页演示（可选合成口播音频）。适用：做视频风格的网页演示、文章转视频脚本、B站/YouTube录屏内容、产品demo、talk演示。每次点击推进一个口播节拍，每步独占整屏。34套内置主题来自 beautiful-html-templates，每套完整 CSS 设计系统。章节开发用 12 个语义 class，主题可随时切换。
---
...
本 Skill **以方法论 + 协作流程为核心**。脚手架提供 34 套 BHT 主题（每套完整
CSS 设计系统），12 个语义 class 作为章节与主题之间的不变桥梁。美学决策
（配色、字型、动效气质）由主题承载，动画编排由章节 agent 自由发挥。
...
- 它是 [`CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md) 这套指引在**当前
  主题 + 当前题材**下的第一次落地
- 如果指引有盲区 / 主题提供的 CSS 变量或 class 覆盖不完整，第 1 章一定会暴露 ——
...
开发。**风格不强求章节间完全一致** —— 主题 CSS 变量和 12 MUST class 兜底视觉
...
2. 章节代码物理分离（每章一个文件夹 / 自己的 CSS 前缀），不会互相
   破坏
3. 12 MUST class（`.display` / `.body` / `.card` / `.divider` 等）
   兜底视觉统一，气质不会跑偏
...
| 9 | 整片同一主题 | 章节间不翻表面色；**颜色走 CSS 变量，组件用 12 MUST class**，动画 / 字号 / 间距章节自由 |
...
| [`references/THEMES.md`](references/THEMES.md) | 选 / 造 / 切主题时 | 主题系统架构 + 内置主题目录 + 创作流程 |
...
| [`themes/`](themes) | Checkpoint Plan / Phase 1.2 时翻 | 内置主题（每个含 `theme.json` + `theme.css` + `design.md`） |
